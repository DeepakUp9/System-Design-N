...
### 1. READ COMMITTED
- **Definition**: A transaction only sees **committed data**.  
...
1. Transaction A:  
   ```sql
   BEGIN;
   UPDATE accounts SET balance = balance - 100 WHERE id = 1; -- deducting
   -- not committed yet
   ```
...
2. Transaction B:
   ...
   -- Sees reduced balance (-100), but txn A may ROLLBACK
   ```

If Transaction A rolls back, B saw inconsistent data (a dirty read).

**With READ COMMITTED:**
Transaction B must wait or will see only the last committed balance, not the uncommitted one.

**Real-world analogy:**
ATM shows you balance. With READ COMMITTED, you never see a "temporary" balance that vanishes after rollback.

### 2. REPEATABLE READ
- **Definition**: Ensures that within the same transaction, multiple reads of the same row return the same result.
- **Prevents**: Dirty reads + Non-repeatable reads.
- **Allows**: Phantom reads (new rows appearing).

#### Example (Non-repeatable Read prevention)

**Without REPEATABLE READ:**
1. Transaction A:
   ```sql
   BEGIN;
...
3. Transaction A again:
   ```sql
   SELECT balance FROM accounts WHERE id = 1; -- returns 600 now
   ```

Same query gave different results inside one transaction → non-repeatable read.

**With REPEATABLE READ:**
Transaction A will see the same snapshot for the row → both reads return 500, even though B committed changes.

**Real-world analogy:**
You're checking your bank statement while logged in. No matter how many times you refresh in the same "session," numbers stay the same until you exit.

### 3. SERIALIZABLE
- **Definition**: The strongest isolation. Transactions behave as if they were executed sequentially (one after another).
- **Prevents**: Dirty reads, non-repeatable reads, phantom reads.
- **Cost**: May cause more rollbacks due to conflicts.
...
3. Transaction A again:
   ...
   ```
   → A "phantom" row appeared mid-transaction.

**With SERIALIZABLE:**
Transaction A's second query will still return 10, as if B's insert didn't exist until A commits.

If A later tries an action based on that count, the DB may force A or B to rollback to avoid inconsistency.

**Real-world analogy:**
Imagine a ticket counter that lets one customer finish all operations fully before the next one starts. No surprises.
