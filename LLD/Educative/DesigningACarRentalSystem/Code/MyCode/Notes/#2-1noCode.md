...
## 10 — Scaling / Advanced Concurrency Options

If throughput grows, consider these alternatives:

### Option A: Allocation Queue
* Use Kafka where requests enqueue and a single consumer assigns vehicles
* Avoids DB contention

### Option B: Optimistic Locking
* Use `@Version` and retry on `OptimisticLockException`

### Option C: Distributed Lock
* Use Redis RedLock for multi-instance safety
* ⚠️ Be careful with correctness guarantees

### Option D: CQRS
* Read database for fast availability search (denormalized)
* Write DB for authoritative reservations

### Option E: Multi-Region
* Design a global allocator with strong consistency
* Leader election or sharding vehicles by branch
