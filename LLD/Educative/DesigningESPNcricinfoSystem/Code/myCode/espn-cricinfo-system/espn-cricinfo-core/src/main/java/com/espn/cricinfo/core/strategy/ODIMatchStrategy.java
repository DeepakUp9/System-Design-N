...
/**
 * Strategy implementation for ODI (One Day International) cricket format.
 * ODI rules: 50 overs per innings, maximum 2 innings, fielding restrictions.
 */
@Slf4j
public class ODIMatchStrategy implements MatchFormatStrategy {

    private static final int MAX_OVERS_PER_INNINGS = 50;
    private static final int MAX_INNINGS = 2;
    private static final int POWERPLAY_OVERS = 10; // First 10 overs with fielding restrictions
    private static final int POWERPLAY_MAX_FIELDERS_OUTSIDE_30 = 2;
    ...
    public boolean isMatchComplete(Match match) {
        ...
        // Match is complete if both teams have batted or all overs completed
    ...
    public boolean canDeclareInnings() {
        return false; // ODI doesn't allow declaration
    }
    ...
    public boolean validateBall(Ball ball) {
        ...
        // Validate runs scored (ODI allows maximum 6 + extras)
    ...
    public ScoringResult calculateScore(Ball ball) {
        if (!validateBall(ball)) {
            log.error("Invalid ball for ODI format: {}", ball);
            throw new IllegalArgumentException("Invalid ball for ODI format");
        }
    ...
    public boolean validateMatchState(Match match) {
        ...
        if (match.getCurrentInningsNumber() > MAX_INNINGS) {
            log.warn("Invalid innings count for ODI: {}", match.getCurrentInningsNumber());
            return false;
        }
        ...
        if (match.getCurrentInnings() != null &&
            match.getCurrentInnings().getTotalOvers() > MAX_OVERS_PER_INNINGS) {
            log.warn("Invalid overs count for ODI innings: {}", match.getCurrentInnings().getTotalOvers());
            return false;
        }
    ...
    /**
     * Validates fielding restrictions during powerplay in ODI.
     *
     * @param overNumber the current over number
     * @param fieldersOutside30 number of fielders outside 30-yard circle
     * @return true if fielding setup is valid
     */
    public boolean validatePowerplayFielding(int overNumber, int fieldersOutside30) {
        if (overNumber <= POWERPLAY_OVERS) {
            return fieldersOutside30 <= POWERPLAY_MAX_FIELDERS_OUTSIDE_30;
        }
        return true; // No restrictions after powerplay
    }

    private boolean isTargetReached(Match match) {
        // In ODI, if chasing team reaches target before 50 overs, match ends
